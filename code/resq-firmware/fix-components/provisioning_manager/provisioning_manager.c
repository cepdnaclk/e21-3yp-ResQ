...
static esp_err_t root_get_handler(httpd_req_t *req)
{
    const char *html =
        ...
        "<style>"
        "body{font-family:Arial;margin:24px;background:#f7f9fc;}"
        "form{max-width:420px;background:white;padding:20px;border-radius:12px;}"
        "input{width:100%;padding:10px;margin:8px 0;box-sizing:border-box;}"
        "button{padding:12px;width:100%;background:#0b63ce;color:white;border:0;border-radius:8px;}"
        "</style>"
        "</head>"
        "<body>"
        ...
        "<input name=\"register_url\" placeholder=\"http://192.168.8.100:18080/api/devices/register\" required>"
        "<label>MQTT Host</label>"
        "<input name=\"mqtt_host\" placeholder=\"192.168.8.100\" required>"
        "<label>MQTT Port</label>"
        "<input name=\"mqtt_port\" type=\"number\" placeholder=\"1883\" required>"
