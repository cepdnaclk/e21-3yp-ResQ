...
static void publish_calibration_result(const char *command_id,
  ...
  /* Prefer stored request_id from calibration_manager; fall back to provided
   * command_id for compatibility */
  const char *reply_id = calibration_manager_get_request_id();
  if (reply_id == NULL || reply_id[0] == '\0') {
    reply_id = command_id != NULL ? command_id : "";
  }

  esp_err_t err = calibration_manager_publish_calibration_result(
      reply_id, status, result, reason_id, state, action_id);
  if (err != ESP_OK) {
    ESP_LOGW(TAG, "Failed to publish calibration result (reply_id=%s) err=%d",
             reply_id, err);
  }
}

resq_state_t
calibration_state_manager_run(network_config_t *network_config,
  ...
  while (calibration_manager_is_running()) {
    system_button_action_t action =
        system_button_manager_poll(RESQ_STATE_CALIBRATING);
    ...
    if (mqtt_manager_wait_for_command(&command, pdMS_TO_TICKS(250)) == ESP_OK) {
      ...
      if (suffix != NULL && strcmp(suffix, "cmd/calibration/cancel") == 0) {
        ...
        if (resq_command_extract_request_id(command.payload, reply_id,
          ...
          ESP_LOGW(
              TAG,
              "Missing request_id for cmd/calibration/cancel; skipping cancel");
          continue;
        }
      ...
      if (suffix != NULL && strcmp(suffix, RESQ_SUFFIX_CMD_TELEMETRY) == 0) {
        telemetry_publisher_handle_sensor_stream_command(
            network_config, RESQ_STATE_CALIBRATING, calibration_config,
            &command, false);
        continue;
      }

      ESP_LOGW(TAG, "Ignoring command during calibration topic=%s",
               command.topic);
    }
  ...
  if (calibration_manager_is_ready()) {
    ...
    if (mqtt_manager_is_connected()) {
      mqtt_manager_publish_status(RESQ_STATE_READY_FOR_SESSION, network_config,
                                  calibration_config, false, "", ip_address);
    }
