...
static esp_err_t session_sensor_task_start(void)
{
    ...
    EventBits_t bits = xEventGroupWaitBits(
        s_sensor_task_events,
        SENSOR_TASK_STARTED_BIT | SENSOR_TASK_FAILED_BIT,
        pdFALSE,
        pdFALSE,
        pdMS_TO_TICKS(SENSOR_TASK_START_TIMEOUT_MS));
                                           ...
                                           const cpr_metrics_snapshot_t *snapshot)
{
    ...
    if (session_id != NULL) {
        strncpy(s_pending_interruption.session_id,
                session_id,
                sizeof(s_pending_interruption.session_id) - 1);
    }

    if (snapshot != NULL) {
        memcpy(&s_pending_interruption.metrics,
               snapshot,
...
esp_err_t session_active_manager_init(void)
{
    ...
    if (s_sensor_task_events == NULL) {
        ...
        if (s_sensor_task_events == NULL) return ESP_ERR_NO_MEM;
    }
                                          ...
                                          const resq_mqtt_command_t *cmd)
{
    ...
    if (metrics_err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to reset CPR metrics: %s", esp_err_to_name(metrics_err));
    ...
    if (sensor_err != ESP_OK) {
        ESP_LOGE(TAG, "Failed to start sensor task: %s", esp_err_to_name(sensor_err));
...
static resq_state_t recover_session_connectivity(
    ...
    const char *ip_address)
{
    ...
    while ((esp_timer_get_time() / 1000) - recovery_started_ms <
        ...
        if (action == SYSTEM_BUTTON_ACTION_TURN_OFF ||
            ...
            return action == SYSTEM_BUTTON_ACTION_TURN_OFF
                ? RESQ_STATE_TURN_OFF
        ...
        if (wifi_manager_is_connected() && mqtt_manager_is_connected()) {
            ...
            if (wifi_manager_get_ip(current_ip, sizeof(current_ip)) != ESP_OK) {
                snprintf(current_ip, sizeof(current_ip), "%s", ip_address ? ip_address : "");
            }
            ...
            ESP_LOGW(TAG,
                     "Connectivity returned but resume publish failed: status=%s heartbeat=%s",
        ...
        if (!wifi_manager_is_connected()) {
            if (last_wifi_attempt_ms == 0 ||
                ...
                esp_err_t err = wifi_manager_reconnect_async(
                    WIFI_MANAGER_DEFAULT_MAX_RETRIES);
                last_wifi_attempt_ms = now_ms;
                if (err != ESP_OK) {
                    ESP_LOGW(TAG,
                             "Wi-Fi reconnect request failed: %s status=%d",
                             esp_err_to_name(err),
            ...
            if (last_mqtt_attempt_ms == 0 ||
                ...
                if (err != ESP_OK) {
                    ESP_LOGW(TAG,
                             "MQTT reconnect request failed: %s status=%d",
                             esp_err_to_name(err),
    ...
    if (session_manager_get_state(&session) == ESP_OK && session.active) {
        strncpy(interrupted_session_id,
                session.session_id,
    ...
    ESP_LOGE(TAG,
             "Connectivity recovery deadline expired; session id=%s is terminally interrupted",
                                        ...
                                        const char *ip_address)
{
    ...
    if (!session_manager_is_active() ||
        !session_active_manager_is_sensor_running() ||
        !buzzer_manager_is_running() ||
        !telemetry_publisher_is_running()) {
        ESP_LOGE(TAG, "Session runtime entered active state without all tasks running");
    ...
    while (true) {
        ...
        if (strcmp(command_suffix, "cmd/session/stop") == 0) {
            ...
            if (cleanup_err != ESP_OK) {
                ESP_LOGE(TAG,
                         "Session runtime cleanup failed: %s",
                         esp_err_to_name(cleanup_err));
    ...
    const char *ip_address)
{
    ...
    if (!s_pending_interruption.event_published) {
        ...
        esp_err_t event_err = mqtt_manager_publish_topic_json(
            RESQ_SUFFIX_EVENTS,
            event);
        if (event_err != ESP_OK) {
            return event_err;
