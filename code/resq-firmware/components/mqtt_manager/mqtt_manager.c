...
static esp_err_t enqueue_complete_command(const char *topic,
  ...
  BaseType_t ok = xQueueSend(s_command_queue, &command, 0);
  if (ok != pdTRUE) {
 ...
 * helper */
static void build_topic_for_suffix(const char *device_id, const char *suffix,
                                   char *out, size_t out_len) {
  /* ignore result here; callers generally don't expect failure for internal
   * topics */
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base,
  ...
  case MQTT_EVENT_CONNECTED: {
    ...
    if (s_client && s_topic_cmd_wildcard[0] != '\0') {
      ...
      ESP_LOGI(TAG, "Subscribed to: %s (msg_id=%d)", s_topic_cmd_wildcard,
               msg_id);
    }
...
esp_err_t mqtt_manager_start(const char *device_id, const char *mqtt_host,
  ...
  (void)resq_mqtt_build_topic(select_device_id_runtime(), RESQ_SUFFIX_CMD_ROOT,
  ...
  EventBits_t bits =
      xEventGroupWaitBits(s_mqtt_events, MQTT_CONNECTED_BIT | MQTT_FAIL_BIT,
...
mqtt_manager_publish_heartbeat(const network_config_t *network_config,
  ...
  char topic[MQTT_MANAGER_TOPIC_MAX_LEN];
  build_topic_for_suffix(s_device_id, RESQ_SUFFIX_HEARTBEAT, topic,
                         sizeof(topic));
...
esp_err_t mqtt_manager_publish_event_json(const char *json_payload) {
  if (!s_connected || json_payload == NULL)
    return ESP_ERR_INVALID_STATE;
...
esp_err_t mqtt_manager_publish_telemetry_json(const char *json_payload) {
  ...
  char topic[MQTT_MANAGER_TOPIC_MAX_LEN];
  build_topic_for_suffix(s_device_id, RESQ_SUFFIX_TELEMETRY, topic,
                         sizeof(topic));
...
esp_err_t mqtt_manager_wait_for_command(resq_mqtt_command_t *command,
  ...
  BaseType_t ok = xQueueReceive(s_command_queue, command, timeout_ticks);

  if (ok == pdTRUE) {
    ESP_LOGI(TAG, "Dequeued MQTT command topic=%s payload=%s", command->topic,
             command->payload);
    return ESP_OK;
  }
