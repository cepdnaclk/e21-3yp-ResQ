#include <stdio.h>
#include <string.h>

#include "provisioning_manager.h"
#include "unity.h"

static network_config_t valid_candidate(const char *ssid)
{
    network_config_t config;
    network_config_set_defaults(&config);
    snprintf(config.wifi_ssid, sizeof(config.wifi_ssid), "%s", ssid);
    snprintf(config.wifi_pass, sizeof(config.wifi_pass), "%s", "test-pass");
    snprintf(config.backend_base_url, sizeof(config.backend_base_url), "%s",
             "http://192.0.2.10:18080");
    (void)network_config_validate(&config);
    return config;
}

TEST_CASE("Provisioning page maps canonical QR fields and aliases", "[provisioning][qr]")
{
    const char *html = provisioning_manager_get_page_html();
    TEST_ASSERT_NOT_NULL(html);
    TEST_ASSERT_NOT_NULL(strstr(html, "new URLSearchParams(window.location.search)"));
    TEST_ASSERT_NOT_NULL(strstr(html, "wifi_ssid: ['wifi_ssid','ssid']"));
    TEST_ASSERT_NOT_NULL(strstr(html,
                                "wifi_pass: ['wifi_pass','wifi_password','password']"));
    TEST_ASSERT_NOT_NULL(strstr(html,
                                "backend_base_url: ['backend_base_url','backend_url','hub_url']"));
    TEST_ASSERT_NOT_NULL(strstr(html, "params.has(aliases[i])"));
    TEST_ASSERT_NOT_NULL(strstr(html, "el.value = params.get(aliases[i])"));
}

TEST_CASE("Provisioning password field stays masked optional and editable", "[provisioning][qr]")
{
    const char *html = provisioning_manager_get_page_html();
    TEST_ASSERT_NOT_NULL(strstr(
        html,
        "<input id='wifi_pass' name='wifi_pass' type='password' autocomplete='current-password'>"));
    TEST_ASSERT_NULL(strstr(html, "console."));
    TEST_ASSERT_NOT_NULL(strstr(
        html, "wifi_pass: document.getElementById('wifi_pass').value"));
}

TEST_CASE("Provisioning accepts empty and nonempty passwords", "[provisioning]")
{
    network_config_t config;
    network_config_set_defaults(&config);

    TEST_ASSERT_EQUAL(ESP_OK, provisioning_manager_parse_payload(
        "{\"wifi_ssid\":\"ResQ Lab\",\"wifi_pass\":\"\","
        "\"backend_base_url\":\"http://192.0.2.10:18080\"}", &config));
    TEST_ASSERT_EQUAL_STRING("", config.wifi_pass);
    TEST_ASSERT_TRUE(network_config_validate(&config));

    TEST_ASSERT_EQUAL(ESP_OK, provisioning_manager_parse_payload(
        "{\"wifi_ssid\":\"ResQ Lab\",\"wifi_pass\":\"p@ss&word=123\","
        "\"backend_base_url\":\"http://192.0.2.10:18080\"}", &config));
    TEST_ASSERT_EQUAL_STRING("p@ss&word=123", config.wifi_pass);
    TEST_ASSERT_TRUE(network_config_validate(&config));
}

TEST_CASE("Provisioning preserves special form password characters", "[provisioning]")
{
    network_config_t config;
    network_config_set_defaults(&config);
    TEST_ASSERT_EQUAL(ESP_OK, provisioning_manager_parse_payload(
        "wifi_ssid=ResQ+Lab&wifi_pass=p%40ss%26word%3D123%2B%25%23%3F+quoted&"
        "backend_base_url=http%3A%2F%2F192.0.2.10%3A18080", &config));
    TEST_ASSERT_EQUAL_STRING("ResQ Lab", config.wifi_ssid);
    TEST_ASSERT_EQUAL_STRING("p@ss&word=123+%#? quoted", config.wifi_pass);
}

TEST_CASE("Provisioning password boundary is safe and transactional", "[provisioning]")
{
    char maximum_password[RESQ_WIFI_PASS_MAX_LEN];
    memset(maximum_password, 'x', sizeof(maximum_password) - 1);
    maximum_password[sizeof(maximum_password) - 1] = '\0';

    char body[300];
    snprintf(body, sizeof(body),
             "{\"wifi_ssid\":\"ResQ\",\"wifi_pass\":\"%s\","
             "\"backend_base_url\":\"http://192.0.2.10\"}",
             maximum_password);

    network_config_t config;
    network_config_set_defaults(&config);
    TEST_ASSERT_EQUAL(ESP_OK,
                      provisioning_manager_parse_payload(body, &config));
    TEST_ASSERT_EQUAL_STRING(maximum_password, config.wifi_pass);

    char overlong_password[RESQ_WIFI_PASS_MAX_LEN + 1];
    memset(overlong_password, 'y', sizeof(overlong_password) - 1);
    overlong_password[sizeof(overlong_password) - 1] = '\0';
    snprintf(body, sizeof(body),
             "{\"wifi_ssid\":\"ResQ\",\"wifi_pass\":\"%s\","
             "\"backend_base_url\":\"http://192.0.2.10\"}",
             overlong_password);

    network_config_t before = config;
    TEST_ASSERT_NOT_EQUAL(ESP_OK,
                          provisioning_manager_parse_payload(body, &config));
    TEST_ASSERT_EQUAL_MEMORY(&before, &config, sizeof(config));

    TEST_ASSERT_NOT_EQUAL(ESP_OK, provisioning_manager_parse_payload(
        "wifi_ssid=ResQ&wifi_pass=bad%GG&backend_base_url=http%3A%2F%2Fhub",
        &config));
    TEST_ASSERT_EQUAL_MEMORY(&before, &config, sizeof(config));
}

TEST_CASE("Provisioning submit updates pending state atomically",
          "[provisioning][concurrency]")
{
    TEST_ASSERT_EQUAL(ESP_OK, provisioning_manager_test_reset());
    network_config_t candidate = valid_candidate("atomic-submit");
    char ack_id[16] = {0};
    TEST_ASSERT_EQUAL(
        ESP_OK,
        provisioning_manager_test_submit(&candidate, ack_id, sizeof(ack_id)));

    provisioning_status_t status = {0};
    TEST_ASSERT_EQUAL(ESP_OK, provisioning_manager_get_status(&status));
    TEST_ASSERT_EQUAL(PROVISIONING_STATE_WAITING_FOR_ACK, status.state);
    TEST_ASSERT_TRUE(status.waiting_for_ack);
    TEST_ASSERT_EQUAL_UINT32(1, status.request_generation);
    TEST_ASSERT_FALSE(status.saved_config_available);
}

TEST_CASE("Provisioning ACK commits matching generation",
          "[provisioning][concurrency]")
{
    TEST_ASSERT_EQUAL(ESP_OK, provisioning_manager_test_reset());
    network_config_t candidate = valid_candidate("matching-generation");
    char ack_id[16] = {0};
    TEST_ASSERT_EQUAL(
        ESP_OK,
        provisioning_manager_test_submit(&candidate, ack_id, sizeof(ack_id)));
    TEST_ASSERT_EQUAL(ESP_OK,
                      provisioning_manager_test_commit_ack(ack_id));

    network_config_t saved = {0};
    bool available = false;
    TEST_ASSERT_EQUAL(
        ESP_OK,
        provisioning_manager_take_saved_config(&saved, &available));
    TEST_ASSERT_TRUE(available);
    TEST_ASSERT_EQUAL_STRING(candidate.wifi_ssid, saved.wifi_ssid);
}

TEST_CASE("Provisioning stale ACK cannot commit newer request",
          "[provisioning][concurrency]")
{
    TEST_ASSERT_EQUAL(ESP_OK, provisioning_manager_test_reset());
    network_config_t first = valid_candidate("first");
    network_config_t second = valid_candidate("second");
    char first_ack[16] = {0};
    char second_ack[16] = {0};
    TEST_ASSERT_EQUAL(
        ESP_OK,
        provisioning_manager_test_submit(&first, first_ack,
                                         sizeof(first_ack)));
    TEST_ASSERT_EQUAL(
        ESP_OK,
        provisioning_manager_test_submit(&second, second_ack,
                                         sizeof(second_ack)));
    TEST_ASSERT_NOT_EQUAL(
        ESP_OK, provisioning_manager_test_commit_ack(first_ack));
    TEST_ASSERT_EQUAL(ESP_OK,
                      provisioning_manager_test_commit_ack(second_ack));

    network_config_t saved = {0};
    bool available = false;
    TEST_ASSERT_EQUAL(
        ESP_OK,
        provisioning_manager_take_saved_config(&saved, &available));
    TEST_ASSERT_TRUE(available);
    TEST_ASSERT_EQUAL_STRING("second", saved.wifi_ssid);
}

TEST_CASE("Provisioning NVS failure does not report saved config",
          "[provisioning]")
{
    TEST_ASSERT_EQUAL(ESP_OK, provisioning_manager_test_reset());
    provisioning_manager_test_set_save_result(ESP_FAIL);
    network_config_t candidate = valid_candidate("nvs-failure");
    char ack_id[16] = {0};
    TEST_ASSERT_EQUAL(
        ESP_OK,
        provisioning_manager_test_submit(&candidate, ack_id, sizeof(ack_id)));
    TEST_ASSERT_EQUAL(ESP_FAIL,
                      provisioning_manager_test_commit_ack(ack_id));

    provisioning_status_t status = {0};
    TEST_ASSERT_EQUAL(ESP_OK, provisioning_manager_get_status(&status));
    TEST_ASSERT_EQUAL(PROVISIONING_STATE_WAITING_FOR_ACK, status.state);
    TEST_ASSERT_FALSE(status.saved_config_available);
    TEST_ASSERT_EQUAL(ESP_FAIL, status.last_error);
}

TEST_CASE("Provisioning take saved config is atomic",
          "[provisioning][fsm]")
{
    TEST_ASSERT_EQUAL(ESP_OK, provisioning_manager_test_reset());
    network_config_t candidate = valid_candidate("atomic-take");
    char ack_id[16] = {0};
    TEST_ASSERT_EQUAL(
        ESP_OK,
        provisioning_manager_test_submit(&candidate, ack_id, sizeof(ack_id)));
    TEST_ASSERT_EQUAL(ESP_OK,
                      provisioning_manager_test_commit_ack(ack_id));

    network_config_t saved = {0};
    bool available = false;
    TEST_ASSERT_EQUAL(
        ESP_OK,
        provisioning_manager_take_saved_config(&saved, &available));
    TEST_ASSERT_TRUE(available);
    TEST_ASSERT_EQUAL_MEMORY(&candidate, &saved, sizeof(saved));
}

TEST_CASE("Provisioning saved notification is consumed once",
          "[provisioning][fsm]")
{
    TEST_ASSERT_EQUAL(ESP_OK, provisioning_manager_test_reset());
    network_config_t candidate = valid_candidate("consume-once");
    char ack_id[16] = {0};
    TEST_ASSERT_EQUAL(
        ESP_OK,
        provisioning_manager_test_submit(&candidate, ack_id, sizeof(ack_id)));
    TEST_ASSERT_EQUAL(ESP_OK,
                      provisioning_manager_test_commit_ack(ack_id));

    network_config_t saved = {0};
    bool available = false;
    TEST_ASSERT_EQUAL(
        ESP_OK,
        provisioning_manager_take_saved_config(&saved, &available));
    TEST_ASSERT_TRUE(available);
    TEST_ASSERT_EQUAL(
        ESP_OK,
        provisioning_manager_take_saved_config(&saved, &available));
    TEST_ASSERT_FALSE(available);
    TEST_ASSERT_EQUAL_STRING("consume-once", saved.wifi_ssid);
}

TEST_CASE("Provisioning stop rejects new submissions",
          "[provisioning][lifecycle]")
{
    TEST_ASSERT_EQUAL(ESP_OK, provisioning_manager_test_reset());
    TEST_ASSERT_EQUAL(ESP_OK, provisioning_manager_test_set_stopping());
    network_config_t candidate = valid_candidate("rejected");
    char ack_id[16] = {0};
    TEST_ASSERT_EQUAL(
        ESP_ERR_INVALID_STATE,
        provisioning_manager_test_submit(&candidate, ack_id, sizeof(ack_id)));
}

TEST_CASE("Provisioning status snapshot is internally consistent",
          "[provisioning]")
{
    TEST_ASSERT_EQUAL(ESP_OK, provisioning_manager_test_reset());
    provisioning_status_t status = {0};
    TEST_ASSERT_EQUAL(ESP_OK, provisioning_manager_get_status(&status));
    TEST_ASSERT_EQUAL(PROVISIONING_STATE_RUNNING, status.state);
    TEST_ASSERT_TRUE(status.running);
    TEST_ASSERT_FALSE(status.waiting_for_ack);
    TEST_ASSERT_FALSE(status.saved_config_available);

    network_config_t candidate = valid_candidate("status");
    char ack_id[16] = {0};
    TEST_ASSERT_EQUAL(
        ESP_OK,
        provisioning_manager_test_submit(&candidate, ack_id, sizeof(ack_id)));
    TEST_ASSERT_EQUAL(ESP_OK, provisioning_manager_get_status(&status));
    TEST_ASSERT_EQUAL(PROVISIONING_STATE_WAITING_FOR_ACK, status.state);
    TEST_ASSERT_TRUE(status.running);
    TEST_ASSERT_TRUE(status.waiting_for_ack);
    TEST_ASSERT_FALSE(status.saved_config_available);
}
